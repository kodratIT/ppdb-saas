...
- **Endpoint**: `/api/webhooks/xendit` (Global endpoint) or `/[tenant]/api/webhooks/xendit`?
  - _Decision:_ Global endpoint is easier to configure in Xendit Dashboard. We can route based on `external_id` prefix (e.g., `invoice_{tenantId}_{appId}_{timestamp}`).
...
2.  **Credential Management**: For this SaaS, does each School have their own Xendit keys, or does the SaaS owner (Kodrat) collect all money and disburse it (Platform model)?
    - _Assumption for MVP:_ Single Xendit Account (Kodrat's) for simplicity, or we store API keys in `tenants` table if schools have their own.
    - _Recommendation:_ **Per-Tenant API Keys**. Add `xendit_secret_key` column to `tenants` (encrypted). This makes it true SaaS where schools get their money directly.
