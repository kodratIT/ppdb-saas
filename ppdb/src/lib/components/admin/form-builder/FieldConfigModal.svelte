<script lang="ts">
	...
	let {
		...
		open = $bindable(),
		onsave
	}: { field: any; open: boolean; onsave: () => void } = $props();

	// Local state for editing
	let editedField = $state<any>(null);
	...
	function removeOption(index: number) {
		editedField.options = editedField.options.filter((_: any, i: number) => i !== index);
	}
</script>
					...
					<div class="space-y-3 border-t pt-4">
						...
						<div class="max-h-[160px] space-y-2 overflow-y-auto pr-1 custom-scrollbar">
							{#if editedField.options?.length > 0}
								{#each editedField.options as option, i}
									<div class="flex items-center gap-2">
