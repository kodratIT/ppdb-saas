...
<header class="bg-white dark:bg-gray-800 shadow-sm h-16 flex items-center justify-between px-8 z-10 sticky top-0">
	...
	<div class="flex items-center space-x-4">
		...
		<div class="relative">
            ...
            {#if showNotifications}
                <div class="origin-top-right absolute right-0 mt-2 w-80 rounded-md shadow-lg py-1 bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 focus:outline-none z-20">
                    <div class="px-4 py-2 border-b border-gray-100 dark:border-gray-700">
                        <h3 class="text-sm font-semibold text-gray-900 dark:text-white">Notifications</h3>
                    </div>
                    <div class="max-h-64 overflow-y-auto">
                        {#each notifications as note}
                            <div class="px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 border-b border-gray-100 dark:border-gray-700 last:border-0 cursor-pointer {note.read ? 'opacity-60' : ''}">
                    ...
                    <div class="px-4 py-2 border-t border-gray-100 dark:border-gray-700 text-center">
                        <a href="/admin/notifications" class="text-xs font-medium text-blue-600 hover:text-blue-500 dark:text-blue-400">View all notifications</a>
                    </div>
                </div>
                <!-- Backdrop to close -->
                <button class="fixed inset-0 z-10 w-full h-full cursor-default focus:outline-none" on:click={() => showNotifications = false} aria-label="Close notifications" tabindex="-1"></button>
            {/if}
		</div>
	</div>
</header>
