<script lang="ts">
	...
	import { Search, Filter, ChevronLeft, ChevronRight, FileText, User, Globe } from 'lucide-svelte';
...
<div class="container mx-auto px-4 py-8 max-w-6xl">
	<div class="flex items-center justify-between mb-8">
		<div>
			<h1 class="text-3xl font-bold tracking-tight">{i18n.t('admin.auditLogs.title')}</h1>
			<p class="text-muted-foreground mt-1">
				{i18n.t('admin.auditLogs.subtitle')}
			</p>
		</div>
	</div>
			...
			<div class="grid gap-4 md:grid-cols-4 items-end">
				<div class="space-y-2">
					<label
						...
						>{i18n.t('admin.auditLogs.actionType')}</label
					>
					<select
						id="action"
				...
				<div class="space-y-2">
					<label
						for="from"
						class="text-xs font-semibold uppercase tracking-wider text-muted-foreground"
						>{i18n.t('admin.auditLogs.fromDate')}</label
					>
				...
				<div class="space-y-2">
					<label
						for="to"
						class="text-xs font-semibold uppercase tracking-wider text-muted-foreground"
						>{i18n.t('admin.auditLogs.toDate')}</label
					>
