<script lang="ts">
	...
	import { Shield, Users, Plus } from 'lucide-svelte';
	import { i18n } from '$lib/i18n/index.svelte';
	import AdminPageHeader from '$lib/components/admin/AdminPageHeader.svelte';

	let { data }: { data: PageData } = $props();

	const roles = ['super_admin', 'school_admin', 'verifier', 'treasurer', 'interviewer', 'field_officer', 'parent'];
	...
	const getStatusText = (status: string) => {
		switch (status.toLowerCase()) {
			case 'active': return i18n.t('admin.roles.active');
			case 'inactive': return i18n.t('admin.roles.inactive');
			case 'pending': return i18n.t('admin.roles.pending');
...
<AdminPageHeader
	title={i18n.t('admin.userManagement.title')}
	description={i18n.t('admin.userManagement.subtitle')}
>
	{#snippet actions()}
		<Button>
