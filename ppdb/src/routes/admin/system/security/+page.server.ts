...
export const load: PageServerLoad = async ({ url, locals }) => {
	...
	const [failedLoginsResult] = await db
		.select({ count: sql<number>`cast(count(*) as integer)` })
		.from(auditLogs)
		.where(like(auditLogs.action, '%failed%'));

	const [suspiciousActivityResult] = await db
		.select({ count: sql<number>`cast(count(*) as integer)` })
		.from(auditLogs)
		.where(and(like(auditLogs.action, '%delete%'), gte(auditLogs.createdAt, new Date(Date.now() - 7 * 24 * 60 * 60 * 1000))));
	...
	return {
		...
		stats: {
			...
			suspiciousActivity: suspiciousActivityResult.count
		}
	};
};
