...
<div class="container max-w-2xl mx-auto py-8 px-4">
	<div class="mb-6">
		<a href="/{data.tenantSlug}/admin/verify/{data.application.id}" class="text-sm text-primary hover:underline">
				...
				<Card.Content class="space-y-4">
					<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
						...
						<div class="space-y-2">
							...
							<Input id="floor" name="floor" placeholder="Misal: Keramik, Semen, Tanah..." required />
						</div>
						<div class="space-y-2">
			...
			<Card.Root>
				...
				<Card.Content class="space-y-4">
					<div class="grid grid-cols-2 gap-4">
						...
						<label class="border-2 border-dashed rounded-lg h-32 flex flex-col items-center justify-center cursor-pointer hover:bg-muted/50 transition-colors">
							<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-muted-foreground mb-2"><path d="M14.5 4h-5L7 7H4a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-3l-2.5-3z"/><circle cx="12" cy="13" r="3"/></svg>
							<span class="text-xs text-muted-foreground">Tambah Foto</span>
							<input type="file" accept="image/*" multiple on:change={handleFileChange} class="hidden" />
						</label>
					</div>
				</Card.Content>
			</Card.Root>
			...
			<Card.Root>
				...
				<Card.Content class="space-y-4">
					<div class="space-y-2">
						...
						<Textarea id="summary" name="summary" placeholder="Tuliskan detail temuan di lapangan..." required />
					</div>
					<div class="space-y-2">
						<Label for="recommendation">Rekomendasi</Label>
						<select name="recommendation" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50" required>
