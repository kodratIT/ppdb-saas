...
### Database Schema: Global vs Tenant Users
...
erDiagram
    ...
    user_registrations {
        ...
        json student_data
        json parent_data
        json school_origin_data
        json path_specific_data
        string status
        timestamp created_at
    }
    ...
    global_users ||--o{ user_registrations : registers
    tenants ||--o{ user_registrations : receives
```
...
sequenceDiagram
    ...
    Note over Client,Clerk: Email/password or OAuth
    
    Clerk->>Clerk: Authenticate User
    
    alt Authentication Success
        Clerk-->>-Client: JWT Token + Session
        Note over Clerk,Client: { token, userId }
        ...
        Note over Clerk,API: { userId, email, metadata }
